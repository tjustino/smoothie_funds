...
class PendingUserTest < ActiveSupport::TestCase
  test "should create pending user" do
    pending_user = PendingUser.new(account: @some_account, email: "user@domain.tld")
    assert pending_user.valid?
  end

  test "account must be unique" do
    pending_user = PendingUser.new(account_id: @some_pending_user.account_id, email: "user@domain.tld")
